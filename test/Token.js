    ...
    describe("Sending Tokens", () => {
        ...
        describe("Success", () => {
            it("transfers token balances", async () => {
                const { token, deployer, recever } = await loadFixture(deployTokenFixture)

                const transaction = await token.connect(deployer).transfer(recever.address, AMOUNT)
                await transaction.wait()

                expect(await token.balanceOf(deployer.address)).to.equal(tokens(999900))
                expect(await token.balanceOf(recever.address)).to.equal(AMOUNT)
            })
            ...
            it("emits a transfer event", async () => {
                ...
                await expect(transaction).to.emit(token,"Transfer")
                    .withArgs(deployer.address, recever.address, AMOUNT)
            })
        })
        ...
        describe("Failure", () => {
            it("rejects insufficient balances", async () => {
                ...
                await expect(token.connect(deployer).transfer(recever.address, INVALID_AMOUNT))
